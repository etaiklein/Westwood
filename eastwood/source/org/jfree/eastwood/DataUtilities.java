    ...
    public static List parseTextData(String dataStr, float[] scaling) {
        ...
        while (iterator.current() != CharacterIterator.DONE) {
            ...
            if (c >= '0' && c <= '9' || c == '-' || c == '.') {
            	...
            	float val = (Float.parseFloat(dataStr.substring(baseIndex,
            			iterator.getIndex())) - min)/ (max - min);
            	if (val >= 0.0f) {
                    series.add(new Float(Math.min(val, 1.0f)));
            	}
            	else {
            		series.add(null);
            	}
                ...
                series.add(new Float((Float.parseFloat(dataStr.substring(
                        baseIndex, iterator.getIndex())) - min) / (max - min)));
                baseIndex = iterator.getIndex() + 1;
                dataset.add(series);
                series = new java.util.ArrayList();
                iterator.next();
